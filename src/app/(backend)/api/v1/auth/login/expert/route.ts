...
async function handleExpertLogin(request: NextRequest, context: RouteHandlerContext) {
  // Extract and validate request body
  const body = await getRequestBody(request);
  
  // Add role parameter for expert login
...
const handlers = createSingleMethodHandler('POST', handleExpertLogin);
