...
async function handleUpdateProgress(request: NextRequest, context: RouteHandlerContext) {
  const moduleId = context.params?.id as string;
  return learningMaterialsController.updateProgress(request, moduleId);
}
