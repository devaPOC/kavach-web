...
import { LEGAL_DOCUMENTS, type ExpertLegalDocumentType } from '@/lib/constants/legal-documents';

interface LegalAgreementsProps {
  onAgreementChange: (agreements: Record<ExpertLegalDocumentType, boolean>) => void;
  agreements: Record<ExpertLegalDocumentType, boolean>;
  className?: string;
}
...
const LegalAgreements: React.FC<LegalAgreementsProps> = ({
  ...
  const [openDialog, setOpenDialog] = useState<ExpertLegalDocumentType | null>(null);

  const handleAgreementToggle = (documentType: ExpertLegalDocumentType, checked: boolean) => {
  ...
  const handleAcceptFromModal = (documentType: ExpertLegalDocumentType) => {
  ...
  const totalAgreements = expertDocuments.length;
  const acceptedCount = expertDocuments.filter(doc => agreements[doc]).length;

  return (
    <div className={`space-y-4 ${className}`}>

      <div className="space-y-3">
        {expertDocuments.map((documentType) => {
