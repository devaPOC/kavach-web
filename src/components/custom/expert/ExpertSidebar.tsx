				...
				<div className={`flex items-center h-16 px-4 border-b border-slate-100 flex-shrink-0 ${sidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
					{sidebarCollapsed ? (
						...
						<button
							...
							className="p-2.5 rounded-xl text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all duration-200"
						...
						<>
							<div className="flex items-center gap-3">
								...
								<span className="text-base font-semibold text-slate-800 tracking-tight">Expert</span>
							</div>
							<button
								onClick={onSidebarCollapse}
				...
				<div className={`px-4 py-4 border-b border-slate-100 flex-shrink-0 ${sidebarCollapsed ? 'px-3' : ''}`}>
					{!sidebarCollapsed ? (
						...
						<div className="flex justify-center">
							<div className="w-10 h-10 bg-gradient-to-br from-indigo-400 to-indigo-600 rounded-xl flex items-center justify-center shadow-sm" title={`${user.firstName} ${user.lastName}`}>
								<span className="text-sm font-semibold text-white">
									{user.firstName?.[0]}{user.lastName?.[0]}
								</span>
							</div>
						...
						<div className="space-y-1">
							{sidebarItems.map((item) => {
								...
								return (
									...
									>
										<Icon className={`h-5 w-5 flex-shrink-0 transition-colors duration-200 ${sidebarCollapsed ? '' : 'mr-3'} ${active ? 'text-indigo-600' : 'text-slate-400 group-hover:text-slate-600'}`} />
